########
#NOTE: this file was lifted from postreview, and needs some heavy formatting to 
#   be done on it. It doesn't crash, but may not do anyting either. :S
########
import RBUtilities
import os
import re
...
class CVSRepository(Repository):
    ...
    An implementation of Repository for CVS repositories
...
class GitRepository(Repository):
    ...
    def get_info(self):
        ...
        self.head_ref =self.util.execute(['git', 'symbolic-ref', '-q', 'HEAD']).strip()
        ...
        if m:
            ...
            version =self.util.execute(["git", "svn", "--version"], ignore_errors=True)
    ...
    def get_origin(self, default_upstream_branch=None, ignore_errors=False):
        ...
        origin_url =self.util.execute(["git", "config", "remote.%s.url" % upstream_remote],
    ...
    def scan_for_server(self, repository_info):
        ...
        server_url = super(GitRepository, self).scan_for_server(repository_info)
        ...
        self.url =self.util.execute(["git", "config", "--get", "reviewboard.url"],
                      ignore_errors=True).strip()
        ...
        if self.type == "svn":
            ...
            # exists.
            prop = SVNRepository(self.url, self.util).scan_for_server_property(repository_info)
    ...
    def diff(self, args):
        ...
        self.merge_base =self.util.execute(["git", "merge-base", self.upstream_branch,
                                   self.head_ref]).strip()
        ...
        if options.guess_summary and not options.summary:
            options.summary = self.util.execute(["git", "log", "--pretty=format:%s",
    ...
    def make_diff(self, ancestor, commit=""):
        ...
        if self.type == "svn":
            diff_lines =self.util.execute(["git", "diff", "--no-color", "--no-prefix",
                                  "-r", "-u", rev_range],
                                 split_lines=True)
            return self.make_svn_diff(ancestor, diff_lines)
        elif self.type == "git":
            return self.util.execute(["git", "diff", "--no-color", "--full-index",
                            rev_range])
    ...
    def make_svn_diff(self, parent_branch, diff_lines):
        ...
        """
        rev = self.util.execute(["git", "svn", "find-rev", parent_branch]).strip()
...
class MercurialRepository(Repository):
    ...
    def _get_hgsubversion_diff(self, files):
        ...
        return (self.util.execute(["hg", "diff", "--svn", '-r%s:.' % parent]), None)
    ...
    def _get_outgoing_diff(self, files):
        ...
        current_branch = self.util.execute(['hg', 'branch'], env=self._hg_env).strip()
...
class PerforceRepository(Repository):
    ...
    def _path_diff(self, args):
        ...
        for path in args:
            ...
            if not m:
                self.util.die('Path %r does not match a valid Perforce path.' % (path,))
    ...
    def _changenum_diff(self, changenum):
        ...
        if changenum == "default":
            ...
            description = self.util.execute(["p4", "describe", "-s", changenum],
                                  split_lines=True)
        ...
        if cl_is_pending and (v[0] < 2002 or (v[0] == "2002" and v[1] < 2)
            ...
            for line_num, line in enumerate(description):
                ...
                if 'Affected files ...' in line:
                    ...
                    self.util.die("Couldn't find any affected files for this change.")
        ...
        for line in description:
            ...
            if changetype in ['edit', 'integrate']:
                ...
                self.util.die("Unknown change type '%s' for %s" % (changetype, depot_path))
